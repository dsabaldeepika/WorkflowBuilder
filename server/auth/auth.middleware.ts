...
export const canAccessWorkflow = (requiredPermission: 'view' | 'edit') => {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (user.role === UserRole.CREATOR && workflow.createdByUserId !== user.id) {
        ...
        if (workflow.workspaceId) {
          ...
          const userWorkspace = workspaces.find(w => w.id === workflow.workspaceId);
      ...
      if (user.role === UserRole.EDITOR) {
        ...
        if (workflow.workspaceId) {
          const memberList = await storage.getWorkspaceMembers(workflow.workspaceId);
        ...
        return res.status(403).json({ message: 'No workflow-specific permissions yet implemented' });
      }
...
export const canCreateInWorkspace = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = req.user as any;
    if (!user) {
      return res.status(401).json({ message: 'Unauthorized' });
