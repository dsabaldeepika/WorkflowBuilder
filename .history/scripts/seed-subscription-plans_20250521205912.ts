...
async function seedSubscriptionPlans() {
  ...
  // Drop and recreate the subscription_plans table
  await db.execute(`DROP TABLE IF EXISTS subscription_plans`);
  await db.execute(`CREATE TABLE subscription_plans (
    id TEXT PRIMARY KEY,
    name TEXT NOT NULL,
  ...
  for (const plan of plans) {
    await db.insert(subscriptionPlans).values({
      ...
      hasAdvancedIntegrations: plan.hasAdvancedIntegrations,
      hasAiFeatures: plan.hasAiFeatures,
      hasCustomBranding: plan.hasCustomBranding,
      hasPrioritySuppport: plan.hasPrioritySuppport,
