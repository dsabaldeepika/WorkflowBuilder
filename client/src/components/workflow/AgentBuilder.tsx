...
import { fetchNodeTypes } from "@/utils/nodeApi";
...
export function AgentBuilder({
  ...
  const [step, setStep] = useState<
    "template" | "configure" | "capabilities" | "deploy"
  >("template");
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      fetchNodeTypes()
        ...
        .catch((err) => {
          setNodeError("Failed to load node types");
        })
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            <Label htmlFor="advancedMode" className="block">
              Advanced Configuration
            </Label>
            <p className="text-xs text-gray-500">
              Enable advanced settings for this agent
            </p>
          </div>
          <Switch
            id="advancedMode"
        ...
        {isAdvancedMode && (
          <div className="space-y-4 p-4 border rounded-md bg-gray-50">
            ...
            <div className="space-y-2 mt-4">
              <h4 className="text-sm font-semibold">Available Node Types</h4>
              {loadingNodes && (
              ...
              {nodeError && (
                <div className="text-xs text-red-500">{nodeError}</div>
              )}
              <ul className="text-xs text-gray-600">
                {nodeTypes.map((node) => (
                  <li key={node.id} className="flex items-center gap-2">
                    ...
                    <span className="text-gray-400">{node.description}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="flex items-center justify-between p-3 border rounded-md hover:border-primary hover:bg-blue-50 transition-colors">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Access data sources and databases
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between p-3 border rounded-md hover:border-primary hover:bg-blue-50 transition-colors">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Connect with external services via API
              </p>
            </div>
          </div>
          <Switch defaultChecked />
        ...
        <div className="flex items-center justify-between p-3 border rounded-md hover:border-primary hover:bg-blue-50 transition-colors">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Enhanced security features and restrictions
              </p>
            </div>
          </div>
          <Switch defaultChecked />
      ...
      <div className="rounded-md bg-green-50 p-4 flex items-start">
        ...
        <div>
          ...
          <p className="text-xs text-green-700 mt-1">
            The {selectedTemplate?.title} agent has been configured
            successfully. Click "Deploy Agent" to launch it.
          </p>
        </div>
      </div>
      ...
      <div className="rounded-md bg-amber-50 p-4 flex items-start">
        ...
        <div>
          ...
          <p className="text-xs text-amber-700 mt-1">
            This agent will be deployed with the security controls and
            governance settings configured in your Enterprise Core settings.
          </p>
        </div>
      </div>
    </div>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[650px] max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-auto py-3">{renderStepContent()}</div>
