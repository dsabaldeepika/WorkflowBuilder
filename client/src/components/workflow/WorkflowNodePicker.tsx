...
export function WorkflowNodePicker({
  ...
  const [activeCategory, setActiveCategory] =
    useState<NodeCategory>(initialCategory);
    ...
    },
    {
      ...
      triggers: [
        ...
        },
        {
          ...
          name: "New Attachment",
          description:
            "Triggers when a new email with an attachment is received",
        },
      ],
    },
    {
  ...
  const scheduleTriggers: PickerNodeType[] = [
    {
      ...
      name: "Run Once",
      description:
        "Trigger workflow to run one time at a specific date and time",
      icon: "🕒",
      category: "trigger",
      popular: true,
    },
  ...
  const filteredApps = apps.filter((app) => {
    const matchesSearch =
      ...
      app.actions.some((action) =>
        action.name.toLowerCase().includes(searchQuery.toLowerCase())
      ) ||
      app.triggers.some((trigger) =>
  ...
  const filteredUtilityNodes = utilityNodes.filter(
    (node) =>
  ...
  const filteredScheduleTriggers = scheduleTriggers.filter(
    (node) =>
  ...
  const filteredWebhookTriggers = webhookTriggers.filter(
    (node) =>
      node.category === activeCategory &&
      (searchQuery === "" ||
        node.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  // Debug: log all props and state for troubleshooting
  console.log(
    "[WorkflowNodePicker] isOpen:",
    isOpen,
    "activeCategory:",
    activeCategory,
    "activeApp:",
  ...
  // Log filtered apps and nodes
  console.log("[WorkflowNodePicker] filteredApps:", filteredApps);
  console.log(
    "[WorkflowNodePicker] filteredUtilityNodes:",
    filteredUtilityNodes
  );
  console.log(
    "[WorkflowNodePicker] filteredScheduleTriggers:",
    filteredScheduleTriggers
  );
  console.log(
    "[WorkflowNodePicker] filteredWebhookTriggers:",
    filteredWebhookTriggers
  );
      ...
      <DialogContent className="sm:max-w-[700px] p-0 max-h-[85vh] overflow-hidden flex flex-col">
        <DialogHeader className="py-3 px-4 border-b">
          ...
          <DialogDescription>
            {activeApp ? (
              <span>
                Choose an {activeCategory === "trigger" ? "trigger" : "action"}{" "}
                for {activeApp.name}
              </span>
            ) : (
        ...
        <div className="overflow-y-auto flex-1">
          {!activeApp ? (
            <div className="p-4">
                ...
                <Tabs value={activeTab} className="w-full">
                  <TabsList className="w-full mb-4 justify-start bg-muted/40 p-1">
                    ...
                    >
                      <Badge
                        variant="outline"
                        className="text-[10px] py-0 px-1 h-4 rounded-sm bg-blue-500 text-white"
                      >
                        NEW
                      </Badge>
                    </TabsTrigger>
              ...
              {activeCategory === "trigger" &&
                filterByTab(filteredScheduleTriggers).length > 0 && (
                  <div className="mb-6">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {filterByTab(filteredScheduleTriggers).map((node) => (
                        <button
                          key={node.id}
                          onClick={() =>
                            handleSelectNode(node.id, node.category)
                          }
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center">
                              ...
                              {node.new && (
                                <Badge className="ml-2 text-[10px] py-0 bg-blue-500">
                                  New
                                </Badge>
                              )}
                            </div>
              ...
              {activeCategory === "trigger" &&
                filterByTab(filteredWebhookTriggers).length > 0 && (
                  <div className="mb-6">
              ...
              {filteredApps.length === 0 &&
                ...
                filteredScheduleTriggers.length === 0 &&
                filteredWebhookTriggers.length === 0 && (
              ...
              {activeCategory === "trigger" && activeApp.triggers.length > 0 ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-muted-foreground">
                    ...
                    {activeCategory === "trigger" ? "triggers" : "actions"}{" "}
                    available yet
                  </p>
                </div>
              )}
